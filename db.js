...
export async function fetchAllDeals(fieldMap = {}) {
  ...
  while (true) {
    ...
    const deals = data.data.filter(d => d.pipeline_id && d.pipeline_id === await getProjectPipelineId())
    allDeals.push(...deals)
...
export async function getProjectPipelineId() {
  ...
  const data = await res.json()
  const pipeline = data.data?.find(p => p.name === 'Project')
...
export async function discoverFieldMap() {
  ...
  const targets = {
    ...
    'Prospect or Customer': 'prospectOrCustomer',
    'Lead Source': 'leadSource',
    'Lead Source 2': 'leadSource2',
    'Variation': 'variation',
    'Variation Detail': 'variationDetail',
    'System Priced': 'systemPriced',
    'Project Type': 'projectType',
    'Region': 'region',
    'Has MC Secured': 'hasMCSec',
    'Has MC Unsecured': 'hasMCUnsec',
    'Received Date': 'receivedDate',
    'Review Date': 'reviewDate',
    'Deal Priced': 'dealPriced',
    'Lost Reason': 'lostReason',
    'Lead Manual': 'leadManual',
    'Lead Auto': 'leadAuto',
  }
...
function resolveField(deal, fieldInfo) {
  ...
  if (fieldInfo.options && fieldInfo.options[raw]) return fieldInfo.options[raw]
  if (Array.isArray(raw)) return raw.map(v => fieldInfo.options[v] || v).join(', ')
  return raw
}
...
export function normaliseDeal(d, fieldMap) {
  return {
    ...
    organizationName: d.org_name || d.org_id?.name || '',
