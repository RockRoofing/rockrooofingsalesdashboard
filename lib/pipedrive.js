...
export async function fetchAllDeals(fieldMap = {}) {
  ...
  // Build set of org names that have at least one won deal
  // This is used to determine Existing vs Prospect customer type
  const wonOrgNames = new Set(
    allDeals
      .filter(d => d.status === 'won')
      .map(d => d.org_name || (typeof d.org_id === 'object' ? d.org_id?.name : '') || '')
      .filter(Boolean)
  )

  return allDeals.map(d => normaliseDeal(d, fieldMap, wonOrgNames))
}
...
// Determine if customer is Existing (has won deals) or Prospect
// This is calculated from the deals data - passed in as a Set of org names with won deals
function resolveCustomerType(d, wonOrgNames) {
  const org = d.org_name || (typeof d.org_id === 'object' ? d.org_id?.name : '') || ''
  if (!org) return null
  return wonOrgNames.has(org) ? 'Existing' : 'Prospect'
}

export function normaliseDeal(d, fieldMap, wonOrgNames = new Set()) {
  ...
  const stageOrder = d.stay_in_pipeline_stages?.order_of_stages || []
  const everIn1stContact = stageOrder.includes(45) || d.stage_id === 45 || 
    // For bulk fetched deals without stage history, assume all Project pipeline deals 
    // have been through 1st Contact (they all start there)
    d.pipeline_id === 8
  ...
  return {
    ...
    customerType: resolveCustomerType(d, wonOrgNames),
    ...
    salesPerson: resolveField(d, fieldMap.salesPersonField) || d.user_id?.name || d.owner_name || '',
    label: d.label || null,
    over200k: (d.value || 0) >= 200000,
    // Use createdDate as approximation for firstContactDate on historical deals
    // Webhook will update this with accurate date when stage changes occur going forward
    firstContactDate: d.firstContactDate || (d.add_time ? d.add_time.split(' ')[0] : null),
    firstContactApproximate: !d.firstContactDate, // flag to show it's approximate
    receivedDate: d['90c55b79c62d6d9b63ab0a0172ff7c2032b038be_received'] || null,
