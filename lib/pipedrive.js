...
function resolveField(deal, fieldInfo) {
  ...
  if (Array.isArray(raw)) return raw.map(v => fieldInfo.options?.[v] || fieldInfo.options?.[String(v)] || v).join(', ')
  // Handle comma-separated IDs (Pipedrive set fields)
  if (typeof raw === 'string' && raw.includes(',')) {
    return raw.split(',').map(id => fieldInfo.options?.[id.trim()] || fieldInfo.options?.[parseInt(id.trim())] || id.trim()).join(', ')
  }
  // Try direct lookup, then string key, then numeric key
  if (fieldInfo.options) {
    if (fieldInfo.options[raw] != null) return fieldInfo.options[raw]
    if (fieldInfo.options[String(raw)] != null) return fieldInfo.options[String(raw)]
    if (fieldInfo.options[parseInt(raw)] != null) return fieldInfo.options[parseInt(raw)]
  }
  return raw
}
